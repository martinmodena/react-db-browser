...
const ViewRows = (props) => {
    ...
    return (
        <tbody className="ViewRows" onScroll={revaluateDataWindow}>
            {datas.slice(dataWindowMin, dataWindowMax).map((item, index) =>
                <ViewRow key={item.id} parent={props} index={dataWindowMin + index} data={item} deleteRecord={props.deleteRecord}
